...
/*Tips buscar que la mayoría de figuras estén en el origen porque 
sino tendremos que mover al origen rotate, translate
La proyeccion define que se ve 
 */
...
void dibuja(void){
	...
	glPushMatrix();
	    ...
	    glPushMatrix();
	        glTranslatef(0.0, 1.6, 0.0); //esta pila sigue conteniendo la pila del anterior (la rotacion)
	    	glScalef(0.6, 2, 0.5);
			glColor3f(1,0,1);
...
void teclado (unsigned char key, int x, int y)
{
   switch (key) {
      case 27: exit(0);
      case 'M': muslo = (muslo + 5) % 360;
          break;
      case 'm': muslo = (muslo - 5) % 360;
    break;
      case 'A': if (pantorrilla >= 75) pantorrilla = 75;
                else pantorrilla +=  5;
    break;
      case 'a': if(pantorrilla <= -5 ) pantorrilla = -5;
                else pantorrilla -=  5;
    break;
      case 'P': if (pie >= 30) pie = 30;
                else pie +=  5;
    break;
      case 'p': if (pie <= -30) pie = -30;
    else pie -=  5;
